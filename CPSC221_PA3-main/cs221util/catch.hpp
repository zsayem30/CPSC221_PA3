...
#define INTERNAL_CATCH_METHOD_AS_TEST_CASE(QualifiedMethod, ...)               \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                  \
        ...
        Catch::NameAndDesc(__VA_ARGS__), CATCH_INTERNAL_LINEINFO);             \
    } /* NOLINT */                                                             \
    CATCH_INTERNAL_UNSUPPRESS_ETD_WARNINGS
...
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, ...)                        \
    ...
    Catch::AutoReg(Function, CATCH_INTERNAL_LINEINFO,                          \
                   Catch::NameAndDesc(__VA_ARGS__)); /* NOLINT */              \
    CATCH_INTERNAL_UNSUPPRESS_ETD_WARNINGS
...
#define INTERNAL_CATCH_METHOD_AS_TEST_CASE(QualifiedMethod, Name, Desc)        \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                  \
        &QualifiedMethod, "&" #QualifiedMethod,                                \
        Catch::NameAndDesc(Name, Desc), CATCH_INTERNAL_LINEINFO);              \
    } /* NOLINT */                                                             \
    CATCH_INTERNAL_UNSUPPRESS_ETD_WARNINGS
...
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, Name, Desc)                 \
    CATCH_INTERNAL_SUPPRESS_ETD_WARNINGS                                       \
    Catch::AutoReg(Function, CATCH_INTERNAL_LINEINFO,                          \
                   Catch::NameAndDesc(Name, Desc)); /* NOLINT */               \
    CATCH_INTERNAL_UNSUPPRESS_ETD_WARNINGS
...
struct DecomposedExpression {
    ...
    // If more complex check is required then wrap sub-expressions in
    // parentheses.
    template <typename T>
...
struct AssertionInfo {
    AssertionInfo();
    AssertionInfo(char const *_macroName, SourceLineInfo const &_lineInfo,
...
struct AssertionResultData {
    AssertionResultData()
        : decomposedExpression(CATCH_NULL), resultType(ResultWas::Unknown),
          negated(false), parenthesized(false) {}
    ...
    std::string const &reconstructExpression() const {
        if (decomposedExpression != CATCH_NULL) {
            decomposedExpression->reconstructExpression(
...
MatchAllOf<ComparatorT>
MatcherBase<ObjectT, ComparatorT>::operator&&(MatcherBase const &other) const {
    return MatchAllOf<ComparatorT>() && *this && other;
}
template <typename ObjectT, typename ComparatorT>
MatchAnyOf<ComparatorT>
MatcherBase<ObjectT, ComparatorT>::operator||(MatcherBase const &other) const {
...
template <typename T>
Impl::MatchAllOf<T> AllOf(Impl::MatcherBase<T> const &m1,
...
template <typename T>
Impl::MatchAnyOf<T> AnyOf(Impl::MatcherBase<T> const &m1,
                          Impl::MatcherBase<T> const &m2) {
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, expr)                \
    do {                                                                       \
        Catch::ResultBuilder __catchResult(macroName, CATCH_INTERNAL_LINEINFO, \
                                           #expr, resultDisposition);          \
        try {                                                                  \
            CATCH_INTERNAL_SUPPRESS_PARENTHESES_WARNINGS(__catchResult <=      \
                                                         expr)                 \
                .endExpression();                                              \
            CATCH_INTERNAL_UNSUPPRESS_PARENTHESES_WARNINGS                     \
        } catch (...) {                                                        \
...
public:
    ...
    friend bool operator==(const T &lhs, Approx const &rhs) {
        ...
        bool relativeOK =
            ...
            rhs.m_epsilon * (rhs.m_scale + (std::max)(std::fabs(lhs_v),
    ...
    friend bool operator==(double lhs, Approx const &rhs) {
        ...
        bool relativeOK =
            ...
            rhs.m_epsilon * (rhs.m_scale + (std::max)(std::fabs(lhs),
                                                      std::fabs(rhs.m_value)));
        if (relativeOK) {
            return true;
        }
...
struct ConfigData {
    ...
    ConfigData()
        : listTests(false), listTags(false), listReporters(false),
          listTestNamesOnly(false), listExtraInfo(false),
          showSuccessfulTests(false), shouldDebugBreak(false), noThrow(false),
          showHelp(false), showInvisibles(false), filenamesAsTags(false),
          abortAfter(-1), rngSeed(0), verbosity(Verbosity::Normal),
        ...
        while (!remainder.empty()) {
            ...
            if (pos != std::string::npos) {
                ...
                if (remainder[width] == '\n')
                    width--;
                remainder =
                    remainder.substr(0, tabPos) + remainder.substr(tabPos + 1);
            }
...
class CommandLine {
    ...
    public:
        ...
        // Bind a method taking a single, boolean argument (no placeholder
        // string required)
        template <typename C>
        ...
        // Bind a free function taking a single argument - the object to operate
        // on (no placeholder string required)
        template <typename C>
        ...
        // Bind a free function taking a single argument - the object to operate
        // on (requires a placeholder string)
        template <typename C, typename T>
...
public:
    ...
    void optUsage(std::ostream &os, std::size_t indent = 0,
        ...
        for (it = itBegin; it != itEnd; ++it) {
            ...
            for (std::size_t i = 0; i < (std::max)(usage.size(), desc.size());
                 ++i) {
    ...
    void argSynopsis(std::ostream &os) const {
        for (int i = 1; i <= m_highestSpecifiedArgPosition; ++i) {
            ...
            if (it != m_positionalArgs.end())
                ...
                throw std::logic_error("non consecutive positional arguments "
                                       "with no floating args");
        }
    ...
    populate(std::vector<Parser::Token> const &tokens, ConfigT &config) const {
        validate();
    ...
    Text(std::string const &_str,
        ...
        while (it != strEnd) {
            ...
            if (itEnd != strEnd) {
                ...
                do {
                    if (wrappableAfterChars.find(*findIt) !=
                            std::string::npos &&
                        findIt != itEnd) {
                        itEnd = findIt + 1;
                        itNext = findIt + 1;
                        foundWrapPoint = true;
                               ...
                               wrappableBeforeChars.find(*findIt) !=
                                   std::string::npos) {
                        itEnd = findIt;
                        itNext = findIt;
                        foundWrapPoint = true;
                ...
                if (!foundWrapPoint) {
                    // No good wrap char, so we'll break mid word and add a
                    // hyphen
                    --itEnd;
                    itNext = itEnd;
                    suffix = "-";
                } else {
                    while (itEnd > it && wrappableInsteadOfChars.find(
                                             *(itEnd - 1)) != std::string::npos)
                        --itEnd;
                }
            }
...
inline Option<std::size_t> list(Config const &config) {
    ...
    if (config.listTests() ||
        (config.listExtraInfo() && !config.listTestNamesOnly()))
...
public:
    ...
    virtual ITracker *
    findChild(NameAndLocation const &nameAndLocation) CATCH_OVERRIDE {
...
public:
    ...
    static IndexTracker &acquire(TrackerContext &ctx,
        ...
        if (ITracker *childTracker =
            ...
            tracker =
                new IndexTracker(nameAndLocation, ctx, &currentTracker, size);
...
public:
    ...
    Totals runTest(TestCase const &testCase) {
        ...
        do {
            ...
            } while (!m_testCaseTracker->isSuccessfullyCompleted() &&
                     !aborting());
        }
...
public:
    ...
    int run(int argc, wchar_t const *const *const argv) {
        ...
        for (int i = 0; i < argc; ++i) {
            int bufSize = WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, NULL, 0,
                                              NULL, NULL);
...
public: // IRegistryHub
    ...
    getReporterRegistry() const CATCH_OVERRIDE {
        return m_reporterRegistry;
    }
    virtual ITestCaseRegistry const &
    getTestCaseRegistry() const CATCH_OVERRIDE {
    ...
    getTagAliasRegistry() const CATCH_OVERRIDE {
...
AssertionInfo::AssertionInfo()
    : macroName(""), capturedExpression(""),
      resultDisposition(ResultDisposition::Normal), secondArg("") {}
...
AssertionInfo::AssertionInfo(char const *_macroName,
    ...
    : macroName(_macroName), lineInfo(_lineInfo),
      ...
      resultDisposition(_resultDisposition), secondArg(_secondArg) {}
...
std::string capturedExpressionWithSecondArgument(char const *capturedExpression,
                                                 char const *secondArg) {
...
std::string AssertionResult::getExpression() const {
    if (isFalseTest(m_info.resultDisposition))
        return '!' + capturedExpressionWithSecondArgument(
                         m_info.capturedExpression, m_info.secondArg);
    else
        return capturedExpressionWithSecondArgument(m_info.capturedExpression,
                                                    m_info.secondArg);
}
std::string AssertionResult::getExpressionInMacro() const {
    if (m_info.macroName[0] == 0)
        ...
        return std::string(m_info.macroName) + "( " +
               capturedExpressionWithSecondArgument(m_info.capturedExpression,
                                                    m_info.secondArg) +
               " )";
}
...
ResultBuilder::ResultBuilder(char const *macroName,
    ...
    : m_assertionInfo(macroName, lineInfo, capturedExpression,
                      resultDisposition, secondArg),
      m_shouldDebugBreak(false), m_shouldThrow(false), m_guardException(false),
      m_usedStream(false) {}

ResultBuilder::~ResultBuilder() {
#if defined(CATCH_CONFIG_FAST_COMPILE)
    if (m_guardException) {
        stream().oss << "Exception translation was disabled by "
...
void ResultBuilder::captureExpectedException(
    ...
    data.reconstructedExpression = capturedExpressionWithSecondArgument(
        m_assertionInfo.capturedExpression, m_assertionInfo.secondArg);
...
void ResultBuilder::reconstructExpression(std::string &dest) const {
    dest = capturedExpressionWithSecondArgument(
        m_assertionInfo.capturedExpression, m_assertionInfo.secondArg);
}
...
void TagAliasRegistry::add(std::string const &alias, std::string const &tag,
    ...
    if (!m_registry.insert(std::make_pair(alias, TagAlias(tag, lineInfo)))
        ...
        oss << Colour(Colour::Red) << "error: tag alias, \"" << alias
            << "\" already registered.\n"
            << "\tFirst seen at " << Colour(Colour::Red)
            << find(alias)->lineInfo << '\n'
...
public: // IStreamingReporter
    ...
    virtual void
    testRunStarting(TestRunInfo const &testRunInfo) CATCH_OVERRIDE {
        for (Reporters::const_iterator it = m_reporters.begin(),
                                       itEnd = m_reporters.end();
             it != itEnd; ++it)
    ...
    virtual void
    sectionStarting(SectionInfo const &sectionInfo) CATCH_OVERRIDE {
        for (Reporters::const_iterator it = m_reporters.begin(),
                                       itEnd = m_reporters.end();
             it != itEnd; ++it)
...
struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
    ...
    virtual void
    testRunStarting(TestRunInfo const &_testRunInfo) CATCH_OVERRIDE {
    ...
    virtual void
    testCaseStarting(TestCaseInfo const &_testInfo) CATCH_OVERRIDE {
        currentTestCaseInfo = _testInfo;
    }
    virtual void
    sectionStarting(SectionInfo const &_sectionInfo) CATCH_OVERRIDE {
...
struct CumulativeReporterBase : SharedImpl<IStreamingReporter> {
    ...
    assertionEnded(AssertionStats const &assertionStats) CATCH_OVERRIDE {
        ...
        prepareExpandedExpression(
            sectionNode.assertions.back().assertionResult);
        return true;
    }
    virtual void sectionEnded(SectionStats const &sectionStats) CATCH_OVERRIDE {
        assert(!m_sectionStack.empty());
...
class ReporterRegistrar {
    ...
    class ReporterFactory : public SharedImpl<IReporterFactory> {
        ...
        // *** Please Note ***:
        // - If you end up here looking at a compiler error because it's trying
        // to register your custom reporter class be aware that the native
        // reporter interface has changed to IStreamingReporter. The "legacy"
        // interface, IReporter, is still supported via an adapter. Just use
        // REGISTER_LEGACY_REPORTER to take advantage of the adapter. However
        // please consider updating to the new interface as the old one is now
        // deprecated and will probably be removed quite soon!
        // Please contact me via github if you have any questions at all about
        // this. In fact, ideally, please contact me anyway to let me know
        // you've hit this - as I have no idea who is actually using custom
        // reporters at all (possibly no-one!). The new interface is designed to
        // minimise exposure to interface changes in the future.
        virtual IStreamingReporter *create(ReporterConfig const &config) const {
            return new T(config);
        }
...
public:
    ...
    void encodeTo(std::ostream &os) const {
        ...
        for (std::size_t i = 0; i < m_str.size(); ++i) {
            ...
            default:
                // Escape control chars - based on contribution by @espenalb in
                // PR #465 and by @mrpi PR #588
...
public:
    JunitReporter(ReporterConfig const &_config)
        : CumulativeReporterBase(_config), xml(_config.stream()),
          unexpectedExceptions(0), m_okToFail(false) {
        m_reporterPrefs.shouldRedirectStdOut = true;
    }
    ...
    virtual void
    testCaseStarting(TestCaseInfo const &testCaseInfo) CATCH_OVERRIDE {
...
#if defined(CATCH_CONFIG_FAST_COMPILE)
#define REQUIRE(expr)                                                          \
    INTERNAL_CATCH_TEST_NO_TRY("REQUIRE", Catch::ResultDisposition::Normal,    \
                               expr)
...
#define CHECK_THROWS(expr)                                                     \
    INTERNAL_CATCH_THROWS(                                                     \
        "CHECK_THROWS", Catch::ResultDisposition::ContinueOnFailure, "", expr)
